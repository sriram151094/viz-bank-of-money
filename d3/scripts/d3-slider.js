slider = function (min, max, rangeData) {
    ...
    var svg = d3.select("#eventhandler").append('svg').attr("width", w).attr("height", h)
    ...
    var brush = d3.brushX()
        ...
        .on('brush', function (e) {
            ...
            d3.select("#eventhandler").dispatch('change', {detail: {first: (x.invert(s[0]).toFixed(2)), second: (x.invert(s[1]).toFixed(2))} })
        })
